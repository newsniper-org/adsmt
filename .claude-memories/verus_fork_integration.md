...
description: Verus fork (~/verus-fork) consumes lu-smt as an SMT backend via `verus -V adsmt`; the rc.7→rc.21 arc has been driven by closing the gap end-to-end. rc.10 hash-cons; rc.12 T0; rc.13/14 §3.4 Buchberger+F4+plugin; rc.15 §3.1.A-D + §3.2/§3.3 skeletons + §3.4 CLI; rc.16 §3.5.A-G end-to-end + T0'.1-.3; rc.17 promoted §3.5 v0→v0.x + §3.2 dynasm-rs + §3.3 phase 2 + Stålmarck trailing; rc.18 verus-fork rc.17 retry follow-ups; rc.19 verus-fork rc.18 retry follow-ups; rc.20 §3.5.J gate clause-only half; rc.21 verus-fork rc.20 retry's three priorities all land — (1) §3.5.J runtime gate `cdcl_solve_with_model_deadline_with_seed` + `Solver::prepare_cdcl_seed` BCP-trail seeder, (b''') tracer session-boundary fallback covering Unknown / deadline-cancel paths, (c''') the rc.20-profiled allocator-chain hotspot eliminated via `CdclState` atom-key `String → Term` migration (verus_smoke wall-clock 5 955 → 1 923 ms, 67 % reduction).  Next gate stays on verus-fork — §3.5.J full retry against rc.21 with the BCP-fixpoint seed + ~3 s alloc-hotspot removed.
type: project
