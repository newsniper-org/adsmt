...
description: Verus fork (~/verus-fork) consumes lu-smt as an SMT backend via `verus -V adsmt`; the rc.7→rc.17 arc has been driven by closing the gap end-to-end. rc.10 hash-cons; rc.12 T0; rc.13/14 §3.4 Buchberger+F4+plugin; rc.15 §3.1.A-D + §3.2/§3.3 skeletons + §3.4 CLI; rc.16 §3.5.A-G end-to-end + T0'.1-.3 deadline refinement; rc.17 promoted every §3.5 v0 skeleton to v0.x working (real `dump_cdcl_state`/`cdcl_section` cache/recorder hook/checkpoints/event-replay scan/.lutrace v1) + §3.2 dynasm-rs (`KernelStore`/`JitRegistry`/engine integration) + §3.3 phase 2 (dilemma+n-saturate) + `.luart-cdcl` v1.1 Stålmarck trailing section.  Next gate stays on verus-fork — §3.5.H vargo hook + §3.5.I SmtProcess argv + §3.5.J.pre / §3.5.J smoke matrix retries.
type: project
