...
description: Verus fork (~/verus-fork) consumes lu-smt as an SMT backend via `verus -V adsmt`; the rc.7→rc.24 arc has been driven by closing the gap end-to-end. rc.10 hash-cons; rc.12 T0; rc.13/14 §3.4 Buchberger+F4+plugin; rc.15 §3.1.A-D + §3.2/§3.3 skeletons + §3.4 CLI; rc.16 §3.5.A-G end-to-end + T0'.1-.3; rc.17 promoted §3.5 v0→v0.x + §3.2 dynasm-rs + §3.3 phase 2 + Stålmarck trailing; rc.18-19 verus-fork retry follow-ups; rc.20 §3.5.J gate clause-only half; rc.21 verus-fork rc.20 retry's three priorities all land; rc.22 verus-fork rc.21 retry §(d) Arc::ptr_eq fast paths in alpha_eq_rec + Type::eq; rc.23 verus-fork rc.22 retry §4 UF iter().any(alpha_eq) → IndexSet (e''.1/2/3); rc.24 verus-fork rc.23 retry found the rc.23 UF fix held the wall flat — real 97.5%-of-cycles hot site was the identical pattern in `adsmt-quant/src/ematch.rs::TermUniverse` one crate over, plus 4 more cold sites a workspace-wide grep surfaced. (e'''.1) ematch Vec→IndexSet+contains; (e'''.2) engine quant dedup sets; (e'''.3) cold sweep; (e'''.4) "grep workspace-wide" memory lesson.  Verus-fork-predicted Mode C' wall 4 580→~830 ms.  Adsmt-side direct wall measurement still host-environment-limited.  Next gate stays on verus-fork — rc.24 retry to confirm wall + variance; if wall drops but rlimit ≥ 5 s still timeouts, deadline-cascade extension into UF/SLD/quant phase-2 loops (T0''') becomes the next priority.
type: project
