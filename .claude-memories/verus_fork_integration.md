...
description: Verus fork (~/verus-fork) consumes lu-smt as an SMT backend via `verus -V adsmt`; the rc.7→rc.20 arc has been driven by closing the gap end-to-end. rc.10 hash-cons; rc.12 T0; rc.13/14 §3.4 Buchberger+F4+plugin; rc.15 §3.1.A-D + §3.2/§3.3 skeletons + §3.4 CLI; rc.16 §3.5.A-G end-to-end + T0'.1-.3; rc.17 promoted §3.5 v0→v0.x + §3.2 dynasm-rs + §3.3 phase 2 + Stålmarck trailing; rc.18 verus-fork rc.17 retry follow-ups; rc.19 verus-fork rc.18 retry follow-ups; rc.20 §3.5.J gate finally lands — `Solver::restore_cdcl_state_into` consumes the v1 CDCL section's `clauses` field + stashes them on `aot_prelude_clauses` for the per-`(check-sat)` shortcut.  Plus (b'') satisfiability-only CDCL recorder routing + (c'') v0 load audit / Term-set micro-fix.  Next gate stays on verus-fork — §3.5.J full retry against the rc.20 shortcut.
type: project
