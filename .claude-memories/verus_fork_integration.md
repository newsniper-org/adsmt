...
description: Verus fork (~/verus-fork) consumes lu-smt as an SMT backend via `verus -V adsmt`; the rc.7→rc.22 arc has been driven by closing the gap end-to-end. rc.10 hash-cons; rc.12 T0; rc.13/14 §3.4 Buchberger+F4+plugin; rc.15 §3.1.A-D + §3.2/§3.3 skeletons + §3.4 CLI; rc.16 §3.5.A-G end-to-end + T0'.1-.3; rc.17 promoted §3.5 v0→v0.x + §3.2 dynasm-rs + §3.3 phase 2 + Stålmarck trailing; rc.18-19 verus-fork retry follow-ups; rc.20 §3.5.J gate clause-only half; rc.21 verus-fork rc.20 retry's three priorities all land; rc.22 verus-fork rc.21 retry §(d) verus_smoke flamegraph follow-ups — adsmt-core `alpha_eq_rec` + `<Type as PartialEq>::eq` (verus_smoke flamegraph showed 62.16 % + 17.20 % ≈ 79 % of cycles, neither using rc.10 hash-cons Arc::ptr_eq) both got Arc::ptr_eq fast paths.  Verus-fork-predicted wall recovery on verus_smoke Mode C' 5 898 → ~1 300 ms; adsmt-side direct wall measurement host-environment-limited (verus-fork wall numbers were external-SIGTERM-driven).  Next gate stays on verus-fork — rc.22 retry to confirm predicted wall.
type: project
