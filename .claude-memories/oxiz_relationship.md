...
## OxiZ completeness fix — theory frames not re-based after a deep-level check-sat (2026-06-18, on `8ce7ed2`+1)

verus-fork's **consistency-gate poison** (a prior `(push)(check-sat)(pop)` over the quantified prelude turned the next `SAT(F ∧ x!≠0)` into a spurious `unsat`; the abduce A2 flow returned `[]`). Earlier hypotheses were FALSIFIED in order: not the SAT clause-ledger (drains 78→78), and NOT the persistent `mbqi`/`ematch_engine` quantifier caches (a multi-agent workflow recommended scoping those with `ScopedRollback` — "Option A" — but clearing them on `(pop)` did NOT fix the repro). **Empirical localization beat the structural guess**: instrumenting the actual EUF conflict showed `[EUF CONFLICT] diseq 38~206 expl=[only the diseq reason]` — UF says `x!≡0` but the proof forest has NO edge (a union-find ⇄ proof-forest desync). Frame dump at the inner `(pop)`: `context_stack_depth=14` for a repro with ONE `(push)` → **13 leftover SAT decision frames**. ROOT CAUSE: `on_new_level→euf.push()` / `pop_frame→euf.pop()` share the same context stack as SMT-LIB `(push)`/`(pop)`; an inner `(check-sat)` that returns `Sat`/`Unknown` at a deep level (MBQI iteration cap after a `Sat` round) leaves one theory frame per decision level. `Solver::push`/`pop` DO `backtrack_with_phase_saving(0)` the SAT trail — but with the theory DETACHED (it lives in the owning `TheoryManager` between solves), so no `pop_frame` fires and the EUF/arith/bv frames are never unwound. The next `(pop)` unwinds exactly ONE → the inner solve's `x!~0` merge (recorded below all 13 frames) survives. The §4.1 lock-step invariant `|frames|==level+1` held DURING a solve but was never re-established AFTER it. Same CLASS as the [[oxiz_relationship]] simplex pop/tableau bug (#265) and the rc.36-P0 trigger-F stale bound — one layer over, in the EUF union-find. **FIX** (`oxiz-sat/src/solver/mod.rs`, `solve_with_hooks`): after `solve_with_hooks_inner()` and BEFORE detaching the theory, `if decision_level()>0 { backtrack_with_phase_saving(0) }` — textbook incremental-CDCL(T) "return to assertion base", now applied to theory frames too (fires `pop_frame` for every level). Verdict already fixed in `result`; model snapshotted by `save_model` (read via `self.sat.model()`, not the live trail), so re-basing changes neither. Only non-test caller is the `check()` MBQI loop (mod.rs:552); legacy `solve_with_theory` (non-default, slated for removal #262) already calls `on_backtrack(0)` and was left untouched. VALIDATION: focused repro pair via real `lu-smt` → `unknown`/`unknown` (was `unknown`/`unsat`); abduce-EMPTY → non-empty `(not (= x! 0))` like abduce-WORKS; ground EUF+LIA 2000/2000 z3-parity, ground arith fatal=0 (75 tolerated spurious-*sat* unchanged = pre-existing MBQI-pattern set #260), 141 novel shapes SPURIOUS_UNSAT=0; oxiz-sat 618 / oxiz-solver 526+integration / oxiz-theories 1168 / oxiz-core 1174 green; 2 new permanent regressions (`incremental_checksat_pop_does_not_leak_theory_frame` + genuine-unsat companion), validated to flip unsat→sat exactly at the fix. adsmt stays rc.38 (no bump until full `cargo test --workspace --features oxiz` + verus-fork A2 re-run). OxiZ change UNCOMMITTED on disk (user bumps submodule pointer + commits manually). PROCESS LESSON: when a multi-agent structural diagnosis names a locus, FALSIFY it cheaply first (clear-the-suspected-state probe) before building on it; then instrument the ACTUAL conflict (here the EUF `check_conflicts` + frame depths) rather than theorizing across subsystems — the empty-explanation conflict pointed straight at the trail desync.

## eqvars abduce surfacing — goal-relevance search order, NOT typed-API model-guided (2026-06-22, adsmt `26ab129`)

verus-fork's rc.39.2 regression: `(abduce (= (Sub x! y!) 0))` over the full verus prelude HUNG >300 s (was a PASS surfacing `(= x! y!)` on rc.39.1). TWO conflated problems, both resolved on adsmt HEAD (rebuild required): (1) the HANG was already bounded by `fa0d2d4` (per-subset native cap 0.3 s + 20 s global budget); verus-fork's >300 s binary PREDATED that commit (STALE-BINARY TRAP — always check the test binary's mtime vs the fix commit time before diagnosing). (2) The residual: even bounded, the search examined abducibles in DECLARATION order and `(= x! y!)` is the 15th of 19, so the 20 s budget bailed at the ~28th delegation *just after* reaching it (fragile edge). **FIX = goal-relevance search order** (`abduct_goal_relevance` in adsmt-cli/src/main.rs): rank by `2·(shared goal vars) + 1 if top-level positive equality (term_to_smtlib starts_with "(= ")`; `(= x! y!)` → score 5, examined FIRST, found at the ~3rd delegation. Reorders ONLY the search — every candidate still gets the full entailment+consistency check → soundness/minimality untouched. + batch-first `oxiz_inproc` (whole-buffer `execute_script` before per-command fallback; ~0.7 s vs ~0.95 s/subset; the z3-parity-validated path). **TWO REJECTED APPROACHES (durable — do NOT re-attempt):** (a) **typed-API model-guided prune** (get a counterexample model of `F ∧ ¬G` via `Context::get_model`/`eval_in_model`, keep only abducibles it falsifies) is INFEASIBLE over the quantified prelude — `F ∧ ¬G` is itself `unknown`, so there is NO model to read (empirically verified via oxiz CLI). (b) **push/pop incremental delegation** (assert `F` once, `push`/assert `H∧¬G`/`check`/`pop` per subset for O(query-delta)) is UNSOUND in vendored OxiZ — measured a confirmed SPURIOUS `unsat`: after a `(pop)`, the prior scope's `(= x! y!)` merge LEAKED into the next check (same family as SAT-core pop watcher-leak #287 / simplex pop desync #265). So the per-subset check MUST stay a fresh full solve (the sound+complete authority); the relevance order is the model-free way to reach the answer in time. The black-box text seam ("sat"/"unsat"/"unknown" only, via `oxiz_pick_last`) is thus NOT the surfacing blocker here — the typed seam wouldn't help because there's no model and incremental is unsound. The overflow half (the single-subset `assert_eq -rhs` overflow) is the separate `external/oxiz` i128 migration `3d72b96` (pending user pointer bump). Validated: repro surfaces `(= x! y!)` ~21 s, control `unknown` ~2.6 s; scoped suites green + new `abduct_goal_relevance_ranks_goal_equality_highest` unit test. adsmt stays rc.39.3 (advisory abduce quality, no bump). See [[feedback_roundtrip_through_real_producer]] (validated through the real CLI repro) + [[aot_jit_profile_finding]].

## #347 nested-quantifier confident-spurious-sat: native downgrade + OxiZ guard-strip (2026-06-29, AD1 `ff69d88`, OxiZ `ae1fb50`)

`(and (forall x q) (not (forall x q)))` was a confident spurious **sat**, and a confident sat DEFEATS the lu-smt OxiZ delegation (fires only on Unknown). TWO fixes:
- **Native (`adsmt-engine/src/solver.rs`, `check_ground_with_deadline`):** the instantiation loop discovers ONLY top-level positive `forall`s (`quant::partition_quantifiers`); after NNF/skolemize, a `forall` NESTED in propositional structure (`(and (forall x q) ¬q)`) is left an opaque atom whose ∀ semantics go unenforced → SAT sets `forall-atom=true, q=false`. FIX = set `had_unhandled_quantifier` when a surviving lit `contains_quantifier`, downgrade a resulting `Sat`→`Unknown` (relaxation: Unsat sound, Sat can't be justified → delegate), reusing the audited `had_opaque` downgrade. +regression `nested_forall_and_its_negation_is_not_spurious_sat`.
- **OxiZ (`oxiz-solver/src/solver/mod.rs`):** routing these to delegation exposed a PRE-EXISTING OxiZ spurious-UNSAT on a guarded contradictory quantifier `(=> g (∀x.⊥-body))`. The clean-MBQI MAIN solve is CORRECT (`¬Q→¬g→Sat`) but the single-shot **saturation verifier** recorded the BARE instantiated body `φ` (line 732, dropping guard `Q`) and `fresh_ground_resolve` asserted it unconditionally → guarded `Q⇒⊥` became unconditional `⊥` → spurious unsat. FIX = record the GUARDED lemma `Q⇒φ` + `fresh_ground_resolve` abstracts every quantifier node to a fresh Bool prop (`abstract_quants`, ONE per `TermId`, shared between re-asserted originals AND instances) so the guard survives (`(=> p_Q ⊥)`⤳`¬p_Q`). Abstraction OVER-approximates → `abstract-unsat ⇒ real-unsat` (top-level/pigeonhole conflicts still Unsat; top-level ∀'s `p_Q` pinned true by its own asserted abstraction). +3 regressions (`solve_streamed`=incremental, the path that had the bug). The bug manifests on the INCREMENTAL/streaming saturation-verifier path, NOT batch.
- **VALIDATION (randomized z3 quantifier differential, the gate):** delegation/lu-smt path **164/12000 spurious-unsats → 0**; OxiZ-direct(batch) byte-identical before/after (3 pre-existing spurious-sats unchanged); 773 oxiz-solver + 209 engine tests green. STRICT improvement, zero regression.
- **TWO PRE-EXISTING spurious-sats surfaced + filed (NOT this change):** **#349** native EUF predicate-congruence `a=f(a) ∧ p(f a) ∧ ¬p(a)→sat` — **FIXED 2026-06-29 `dfacb85`**: predicate apps `p(a)` were opaque `pos_atoms`/`neg_atoms` OUTSIDE the congruence universe (only same-atom `p(a)∧¬p(a)` caught), so `a=b` never derived `p(a)~p(b)`. FIX = `adsmt-theory/src/uf.rs` `close()` registers the Bool-sorted atoms so the signature pass merges them, + `detect_predicate_polarity_conflict` flags a class carrying both polarities → unsat (the EUF↔Bool interface equality; predicate analog of #336 EUF↔LRA; soundness-monotone — non-congruent `p(a)/p(c)` stays sat; native EUF z3-diff 20000/0, +2 regressions). **#350** OxiZ identity-axiom congruence `∀x.f(x)=x ∧ f(a)=f(b) ∧ f(b)≠a→sat` (OxiZ doesn't instantiate the identity axiom + close congruence) — still pending, confident spurious **sat**.
- **PROCESS LESSON (attribution under a layered stack):** `lu-smt native answers FIRST`; when native returns a confident (wrong) sat, OxiZ delegation never fires — so a "delegation" spurious verdict can actually be NATIVE. Always test `lu-smt native-only` vs `oxiz CLI batch` vs `lu-smt --features oxiz` vs `z3` to localize. The differential's `lu=sat z3=unsat` residuals looked like an OxiZ-fix regression but were native #349. The user's two steering questions ("could z3 be wrong?", "is context not passed to OxiZ?") were both productively WRONG-but-clarifying: z3 verified correct (cvc5+model+proof), and context IS passed (oxiz-direct reproduces) — the real axis was batch-vs-incremental + native-first. See [[feedback_z3_differential_for_unsat_trust]].

## delegation now POSITIVELY verifies nonlinear obligations — folded-¬(H⇒G) nlsat decomposition + tight-logic render (2026-07-02, OxiZ fork `42fcdc3`, AD1 `e844e38`)

The lu-kb delegation could REFUTE a spurious nonlinear sat but never VERIFY a valid nonlinear goal: `x>0 ⊢ x*x>0` came back `Unknown`. Two coupled gaps, both closed:
- **OxiZ fork (`oxiz-theories/src/nlsat.rs`):** the lu-kb driver folds a sequent `H ⊢ G` into a SINGLE assertion `(not (=> H G))`; `extract_poly_atoms`/`extract_real_poly_atoms` only descended `And`+cmp atoms, so the folded `¬(=>)` dropped → empty poly set → `dispatch_nia/nra` returned None → Unknown. FIX = thread a **polarity flag**: negation distributes into the conjunction of literals the NIA/NRA dispatch consumes (`¬(a⇒b)=a∧¬b`, `¬(a∨…)=¬a∧…`, `¬¬x=x`, cmp flips via `base_pos==pol`); a POSITIVE `∨`/`⇒` (a disjunction) still drops. The real extractor gained the Not/Or/Implies/Distinct arms its integer twin already had. §G conjunct-entailment invariant preserved; `sat` trusted only when nothing dropped (⟹ faithful equivalent). Also generalised `nonlinear_opaque_sat` (mod.rs) to key on `term_is_nonlinear` PRESENCE not the logic string (a nonlinear problem under `ALL`/no-logic/mislabeled — e.g. lu-smt replaying `QF_NIA` under `ALL` — still downgrades its opaque `sat`→`unknown`; verus `Mul`/`RMul` wrapper path unaffected — `term_is_nonlinear` is `false` on `Apply`/`Forall`).
- **AD1 render (`adsmt-delegate/src/lib.rs`):** `render_smtlib` emitted `(set-logic ALL)` always, but OxiZ's sound nlsat dispatch only fires on `NIA`/`NRA`. Added a `TheoryFlags` walk → emit `QF_NIA` (pure-int nonlinear) / `QF_NRA` (any real; real-relaxation `unsat` ⟹ integer `unsat`) for a quantifier-free nonlinear obligation; linear/quantified/EUF stay `ALL`. Both are REQUIRED: tight logic engages Path 1 dispatch, polarity threading lets it decompose the folded goal.
- **VALIDATION:** z3-differential 4500 randomized folded/conj/disj NIA+NRA obligations → **0 FALSE_UNSAT, 0 FALSE_SAT** ([[feedback_z3_differential_for_unsat_trust]]); 3 new `nlsat_integration` regressions (valid folded → unsat + 2 soundness guards: invalid-folded not-unsat, positive-disjunction not-spurious-unsat); oxiz-theories 1205/0, oxiz-solver 461/0; lukb-driver `oxiz_delegation_verifies_a_nonlinear_goal_native_cannot` now PASSES (was Unknown). Soundness posture unchanged — delegation still trusts ONLY OxiZ `unsat`, §5 differential intact. **external/oxiz gitlink bump = user's.**
- **LOWERING finding (the other half of "lowering 확장하고 OxiZ 갭 닫기"):** the first-order fragment (prop/EUF/LIA/LRA/**nonlinear-arith**/Bool-quantifiers/non-recursive-Bool-match/datatypes) ALREADY lowers — the nonlinear obligation the fix now verifies already lowered; the blocker was purely OxiZ-side. Remaining lower.rs abstains: (a) genuinely non-first-order (HO application, function-sort ∀/∃/=, universe quantification, bare lambda — correct forever); (b) recursion/induction (recursors, parametric/indexed match — the large #325 frontier); (c) **term-level `ite` over Int/Real** — the one valuable expansion, but adsmt-core has NO `Ite` term so it needs fresh-var lifting, which is soundness-sensitive (capture under binders: an ite under `∀` referencing the bound var can't lift to the flat `extra_hyps` global channel; + goal-vs-hyp polarity) ⇒ a dedicated PRE-VERIFIED slice, not a coupled quick change. PENDING user decision. See [[cas_integration_proposal]], [[cic_hol_lowering]].
