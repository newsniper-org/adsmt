...
**2026-06-25 — the same AOT-trust shape recurs at the TYPED-KERNEL layer (adsmt-ir M3-1), caught at DESIGN time.** Building the [[typed-cic-ir]] AOT-bank (serialize a type-checked CIC `Env`, reload it, check only the query delta), the *first* proposed design was "serialize the checked `Env` state + a content digest + a re-encode check, then trust it." A 4-lens design-review workflow proved this **fatally unsound — the rc.28 S.1-AOT shape one level up**: a digest + re-encode prove the bytes are a *self-consistent encoding*, NOT that they encode a *type-checked* env; the load path would trust the **derived recursor bookkeeping** (`method_template`/`rec_positions`/`rec_member`) verbatim via the raw unchecked `register_inductive`, so a self-consistent-but-wrong bank → an unsound recursor / guard-bypass → FALSE_UNSAT. The general rule this sharpens: **a load/cache/AOT path must re-establish its invariant, not trust a serialized copy of derived state.** The sound design = an **admission JOURNAL**: bank only the checked-admitter *inputs*, and on load **replay them through the same checked admitters** (re-derivation = re-checking), so the bank adds *no* trust beyond the (후검증'd) admitters and a corrupt/incompatible bank fails to re-admit → `BankError` → fall-through (Unknown-safe). "Bank the INPUTS and re-derive the outputs" beats "bank the derived outputs and trust them" — the same lesson as keeping the empty clause sacred and re-arming the downgrade across a wire, now as *re-run the checker on load* instead of *trust the serialized verdict-state*. (Then adversarially re-reviewed: 0 soundness holes; the only finding was a totality crash on a forged `Fix.rec_arg`, not a wrong verdict.) Verus-recorded as `bank_soundness.rs::trusting_load_needs_false_premise`.

**rc.32.2 — the same shape recurs on THEORY atoms (not just nested boolean structure).** verus-fork found native lu-smt returned a confident unsound `sat` for `(and (> x 0) (< x 0))`: arithmetic atoms were abstracted to free booleans (comparison atoms routed by Bool result sort → never reached LinArith; and the theory check routed top-level `lits`, stripping `(and …)` so conjuncts never reached any theory). Fix mirrors S.1: route comparison atoms by operand sort; surface the **forced** (entailed) literals of an asserted conjunction (`collect_forced_literals`, De Morgan duals) — routing only entailed literals keeps `unsat` sound; **two-stage** validate the SAT model's chosen disjunct atoms (→ `Unknown`/delegate on a model-infeasible conflict, never unsound `sat`, since native has no DPLL(T) refinement loop); and the **`had_opaque`→`Unknown` backstop generalized to theory atoms** (a sort-specialized theory ignoring a non-equality atom over its sort → `Unknown`). TWO further process lessons: (1) **after downgrading to `Unknown`→delegate, verify the DELEGATE is itself sound** — the audit found a *separate* soundness bug inside the OxiZ delegate (simplex `pop` didn't restore the pivoted tableau → `sat`-for-unsat; see [[oxiz_relationship]]); a sound downgrade only helps if the backend it delegates to is sound. (2) the recurring meta-pattern across rc.26/27/28/29/32.2: **a verdict computed while ignoring content the engine couldn't interpret** — opaque boolean structure (rc.26-29), then theory atoms (rc.32.2). Grep for every place a model/verdict is formed without checking that ALL non-trivial atoms were actually interpreted by the responsible theory.
