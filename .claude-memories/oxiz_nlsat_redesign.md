...
(1) memory+mirror+commit [DONE] → (2) architecture discussion [**DONE** — this update] → (3) memory+mirror+commit [**THIS STEP**] → (4) write design doc → (5) full implementation (M0). Full design-workflow synthesis saved at `~/research-library/nonlinear-solvers/DESIGN-WORKFLOW-SYNTHESIS.md` (1214 lines: recommendation + scored comparison + all 3 proposals). Papers: [[research-library-nonlinear-solvers]].
