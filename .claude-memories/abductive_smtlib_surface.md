...
**2026-06-19 — `(abduce)` pattern-definition completeness (verus-fork `abduce-ens-pattern-completeness`, adsmt `c9ed6e1`, no bump, on rc.39.1).** A declared abducible `(ens%L args)` whose entailment needs a `:pattern`-triggered DEFINITION axiom to fire didn't surface. ROOT (isolated on small F = just the def axiom): the per-subset check `F ∧ (ens%L args) ∧ ¬G` should be UNSAT and OxiZ decides it so (e-matches `(ens%L x)↦(ens%L c)`, instantiates, refutes), BUT **native's e-matcher fires only for INT-sorted UF apps and MISSES Bool-sorted PREDICATE triggers** → returns a spurious `sat` (model violates the ∀). `Driver::decide_fh` (adsmt-cli) trusted that DECISIVE native `sat` and never delegated (delegation fired only on native `unknown`). FIX = native `sat` is authoritative ONLY on the quantifier-free fragment; over a quantified F (`history_has_quantifier`) `decide_fh` defers to OxiZ, trusting native `sat` only if OxiZ also can't decide (`unsat` always trusted — a refutation is a proof). Sound: entailment surfaces only confirmed `unsat`, consistency drops only proven `unsat`. Minimal repro → `(ensL xc)`; +2 oxiz-gated regressions (theory_abduction.rs). **SCOPE: fixes the native-`sat` (small-F) case ONLY.** Full verus prelude = native→unknown→already-delegating→OxiZ→unknown at scale → UNCHANGED by this routing fix; remains an OxiZ-side MBQI/e-matching scale frontier (same wall z3 hits @60s; needs the full-prelude abduce .smt2 repro to develop — requested in the reply). **DISCOVERED ADJACENT FINDING (unfixed): native returns spurious `sat` on the `:pattern` predicate-def unsat (main `(check-sat)` too) — latent on the full prelude (native→unknown there), real on small quantified-predicate-def inputs. A native predicate-trigger e-matching fix would also make the deductive path fire the pattern directly.** See [[verus_fork_integration]].
