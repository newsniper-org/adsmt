...
SMT-LIB surface — a caller declares the hypothesis vocabulary and asks
for an abduct on a specific goal, then walks the ranked answers:

```smt2
(declare-abducible (> x 0))           ; the vocabulary of allowed fixes
(get-abduct A (>= x 1))               ; cvc5 extension → top abduct
; ⇒ (define-fun A () Bool (> x 0))
(get-abduct-next)                     ; the next ranked abduct, or (fail)
(abduce (>= x 1))                     ; adsmt-native → the full ranked JSON
```
...
| Tests | **1112 green**, 0 ignored, 0 failed |
...
| Workspace version | `1.0.0-rc.39.3` (2026-06-20) |

---

## Roadmap snapshot (rc.39.3 → v1.0.0 stable)
...
| re-parseable `term` in the abductive JSON (verus-fork verify-or-explain design) | **landed** at rc.35.1.  The ranked `(abduce)` JSON rendered `hypotheses` via the engine's curried-HOL Display (`> x 0`, not re-parseable); rc.35.1 routes `hypotheses` + a new top-level `term` (the conjoined abduct, byte-identical to `(get-abduct)`'s define-fun body) through `term_to_smtlib`, so A2a (list) and A2c (back-translate) share one parser.  Additive JSON field, Command enum unchanged.  1 regression; 1089 → **1090** green.  Follow-up: **consistency-enforced abduction** — a derivation-only abduct (`H ⊢ G`) can be inconsistent with the assertions `F` and verify the goal vacuously; opt-in `(set-option :abduct-consistency true)` gives the full cvc5 semantics (`F ∧ H ⊨ G` AND `SAT(F ∧ H)`) by `check-sat`-ing each candidate engine-side — `(abduce)` tags a `consistent` field, `(get-abduct)` drops the inconsistent. 2 regressions; 1090 → **1091** green.  Follow-up: **streaming robustness** — a bad `(abduce …)` term exited lu-smt mid-stream (fatal for verus's persistent-solver pipe); the read-only abductive commands now report + continue (cvc5/z3 `-in` contract), `--strict-commands` still fatal, `(assert)` untouched.  Integration test; 1091 → **1094** green.  Follow-up: **theory-aware abductive search** — the abduce search is syntactic (SLD/α-match), so theory goals (`x>0 ∧ y>0 ⊨ x+y>0`) return []; opt-in `(set-option :abduct-theory true)` runs a bounded minimal-subset search over the declared abducibles for `F ∧ H ⊨ G` under the theory (+ `SAT(F ∧ H)` = full cvc5 `(get-abduct)`).  Opt-in (complementary to SLD's Horn rule base; a `check-sat` per subset).  6 integration regressions; 1094 → **1100** green.  **Typing refinement** (illegal-states-unrepresentable, `Theorem`-kernel discipline on the abductive surface): the search mode is a total `AbductMode` enum (one exhaustive `match`, not scattered bool gates), and a `TheoryAbduct` newtype whose sole constructor runs both `F ∧ H ⊨ G` + `SAT(F ∧ H)` makes an unverified theory abduct unrepresentable.  Pure refactor; 1100 → **1101** green.  **rc.36 — delegate + fix the backend.** `:abduct-theory` worked on native arithmetic but returned `[]` on a real obligation `(> (Add x y) 0)` (`Add` axiomatized), because the per-candidate `check-sat` ran native-only.  rc.36 routes it through the **same OxiZ delegation the top-level `(check-sat)` uses** (`Driver::decide_fh`).  The vendored OxiZ then returned *wrong* verdicts on that axiom (z3-cross-checked), so three OxiZ-side fixes landed (submodule branch `0.2.4-feat/cdqi`): UF-of-int **sort persistence across `execute_script` calls** (a `ParserEnv` threaded as state), **IntConst pinned apart in EUF**, and **pattern-guided MBQI** (e-matching to a fixpoint, model-based pass skips trigger axioms, non-termination backstop) — so the `Add` cases now match z3 and the in-process delegation returns `(>= x 0)` in ~0.01 s.  **CDQI** (conflict-driven instantiation, FMCAD'14) added on the same branch.  4 adsmt + 12 OxiZ regressions; 1101 → **1105** green |
