...
| §3.5.H vargo post-build bake hook | verus-fork side; the **last** wiring step — stages `prelude-<sha>-<lu_smt_version>.luart-cdcl` + sets `VERUS_ADSMT_AOT_LUART`; the per-query AOT win materialises once the hook elides the re-asserted prelude from the per-query stdin payload.  Gates the v1.0 cut (+ explicit user sign-off) |
