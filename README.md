...
| What | Where |
|---|---|
| Project version | `1.0.0-rc.34.6` (testing channel; cuts to `v1.0.0` stable on explicit sign-off) |
...
| Tests | **1083** passing across the workspace; 0 `cargo doc` / `cargo build` warnings (rc.34→.1 built+fixed the §3.5 JIT-on-AOT trace-replay sub-cycle; rc.34.2 the slim verdict-only trace; rc.34.3 the 32-byte clause-set digest; rc.34.4 made that digest **incremental**; rc.34.5 chained the replay's atom map under a precomputed prelude base so the consult is `O(query delta)`; rc.34.6 gated that precompute on a trace actually being loaded, so the AOT-only `--aot-load` path is no longer taxed by a map it never reads) |
...
## Workspace topology (v1.0.0-rc.34.6)
