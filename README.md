...
| What | Where |
|---|---|
| Project version | `1.0.0-rc.39.2` (testing channel; cuts to `v1.0.0` stable on explicit sign-off) |
...
| Tests | **1105** passing across the workspace; 0 `cargo doc` / `cargo build` warnings (rc.34.x closed the §3.5 JIT-on-AOT trace-replay perf arc — slim trace → clause-set digest → incremental digest → precomputed prelude atom map, all `O(query delta)`; rc.35 added the **abductive-reasoning SMT-LIB surface** — `(declare-abducible …)`, `(abduce …)`, and the cvc5-compatible `(get-abduct …)` / `(get-abduct-next)`; rc.36 routes `:abduct-theory`'s per-candidate `check-sat` through **OxiZ delegation** so the search reasons over axiomatized/quantified goals, and fixes the vendored OxiZ's UF-of-int sort + pattern-guided MBQI; rc.37 lands the **vendored OxiZ §4 SAT-core redesign Phase 2** — the lock-step `TheoryHooks` CDCL(T) driver is the in-process-backend default and a pre-existing EUF↔arith spurious-`sat` is fixed; rc.38 **restores the clean-MBQI default's soundness** — making `clean_mbqi` the default exposed a previously-unmeasured spurious-`sat` in the quantifier suite (an instance-groundness guard dropped ground instances over declared-constant `Var` terms), fixed by a bound-var-survival test; plus a SAT-core incremental-probe **dangling-watch** fix that was a pre-existing spurious-`unsat` on recycled clause ids; **rc.39** closes the verus-fork abductive-residual pair — a consistency-gate **theory-frame leak** (a deep-level `(check-sat)` returning at a deep decision level left per-decision EUF/arith frames un-re-based, so the next `(pop)` stranded an in-scope EUF merge → spurious `unsat`; the vendored OxiZ's `solve_with_hooks` now re-bases the trail to decision level 0 before detaching the theory, firing `pop_frame` for every level) and an entailment-gate **streaming-vs-batch divergence** (the `:abduct-theory` delegation rebuilt `F` from the session history and, when fed one command at a time, replayed the prior `(check-sat)` inside a still-open `(push)`; `strip_abductive_commands` now also drops the session's interactive query/output commands so the delegated `F`-query is feed-independent — streaming ≡ batch; **rc.39.1** advances the vendored OxiZ submodule — the clean-MBQI quantifier engine's z3-parity completeness frontier is **closed** (every `z3=Sat` corpus case is now decided `Sat`, 0 spurious, via model-construction recognizers for ∀∃ Skolem-witness equalities, dense-real range / bounded-oscillation / variable-relative / commuting-function / layered bounds, the real fixed-point, the nested `∀∃∀` threshold guard, and triangular array-index guards — plus a capture-avoiding `substitute`-into-quantifier core fix in `oxiz-core`). From rc.39.1 to the `v1.0.0` stable cut the focus narrows to **cross-platform hardening + docs/comments/book**; **rc.39.2** fixes a latent native soundness item flagged by verus-fork — the **Bool-predicate-trigger e-matching** gap: the native e-matcher learned triggers only from arithmetic/UF-of-int atoms, never from a Bool-sorted *predicate* application, so a `:pattern`-annotated definition `∀x. (P x) ⟺ <body>` (verus's `ens%L` axioms) fired no instantiation, and the CNF flattener treated the body's Bool equality `(= (P x) <body>)` as one **opaque atom** rather than the iff `(and (=> · ·) (=> · ·))` — together they let a quantified predicate-definition `(check-sat)` return a spurious `sat` (the model violated the un-instantiated `∀`). Fixed in two places: `collect_trigger_patterns` now recurses through equality so `(P x)` becomes a trigger, and `rewrite_bool_iff` in the CNF flattener encodes every Bool-sorted `=` as the propositional iff (an EUF/arith equality stays a theory atom — `dest_iff` gates on the operand sort). The main `(check-sat)` on the small predicate-definition shape is now sound (`unsat`), and the abduce deductive path fires the `:pattern` definition directly; the full-prelude heavy-cut stays OxiZ-MBQI-scale-bound (~75 s, the same wall z3 hits), as scoped) — `1112` adsmt-workspace tests (OxiZ is an excluded sub-workspace; its own suites green, zero spurious sat/unsat) |
...
## Workspace topology (v1.0.0-rc.39.2)
...
| rc.38 | **Clean-MBQI default soundness fully restored + a SAT-core incremental-probe fix.** Two soundness classes closed in the vendored OxiZ (submodule `external/oxiz`, branch `0.2.4-redesign`, `369a3a8`→`0d08274`). **(1) SAT-core — recycled-clause-id dangling watch.** `forget_learned_since` freed clause ids to the recycling free-list but left their watchers dangling; the bit-vector theory's incremental `check()` probe frees an id and *immediately* reuses it via the next `add_clause` (no intervening propagation), and `propagate`'s "skip deleted clause" guard is defeated the moment `add` recycles the slot and clears its `deleted` flag — so the recycled clause inherited the forgotten clause's watchers and mis-propagated → a spurious `unsat`. Now detaches each forgotten clause's watchers (`WatchLists::remove_clause`) before freeing the id; a disjoint-field borrow keeps it allocation-free. Fixes the pre-existing `bv_mul_aux_disjunction_const_is_sat_8bit` (`a=x*3 ∧ a≠x ∧ (a=x ∨ a=7)`, witness x=173, was `unsat`). **(2) clean-MBQI — spurious `sat` exposed by the default flip.** Making `config.clean_mbqi` the verifier default (the trigger-F spurious-`unsat` P0 fix that flips the engine) surfaced a previously-**unmeasured** spurious *SAT* in the quantifier-soundness suite — never run in the prior partial validation. The clean engine's instance-groundness guard rejected on `free_vars(phi)` being non-empty, but **OxiZ models declared constants as `Var` terms**, so a perfectly ground instance like `Add(x,y)=x+y` over declared constants `x,y` was DROPPED, losing the lemma that entails the conflict → premature `Saturated` → a spurious `sat` on UNSAT problems (`Add(x,y)=7 ∧ ∀a b.Add(a,b)=a+b ∧ x+y=9`). Now rejects only when one of THIS quantifier's **bound** variables survives substitution. **Completeness:** `eval_forall` implemented for the structural-tautology fragment (`t=t`, `t≤t`, `t≥t`, ∧/∨/⇒/ite/¬), returning `Some(true)` ONLY for model-INDEPENDENT validities (never from model-sample agreement) — recovers `Sat` on trivially-valid trigger-free axioms (`∀x.f(x)=f(x)`) with zero spurious-`sat` risk. Two satisfiable-but-not-valid trigger-free cases (`∀x.partial-order(x,x)`; `∀a.f(a)>0` with consistent `f(7)=5`) now report the SOUND `Unknown` — full `Sat` needs M3 model completion (M4e follow-up); their tests relaxed to `assert_ne(Unsat)` (both `Sat` and `Unknown` sound). z3-cross-checked; OxiZ suites green, **zero spurious sat/unsat** across the quantifier-soundness + bv-soundness suites. adsmt-side: submodule pointer bumped; the fixes are OxiZ-side so the adsmt-workspace test count is unchanged. **1105** green. |
| rc.37 | **Vendored OxiZ §4 SAT-core redesign Phase 2 — the lock-step `TheoryHooks` driver is the in-process-backend default, and a pre-existing EUF↔arith spurious-`sat` is fixed.** The in-process OxiZ backend (what `lu-smt --features oxiz` / `-V adsmt` delegates to when native returns `unknown`) landed the §4 CDCL(T) theory-integration rewrite (submodule `external/oxiz`, branch **`0.2.4-redesign`**, `012b726`→`369a3a8`): an **owning `TheoryManager`** driven by the lock-step **`TheoryHooks`** contract instead of the advisory `TheoryCallback`. `\|theory frames\| == decision_level + 1` now holds *by construction* — the trail fires `push_frame`/`pop_frame` atomically with each level move and `assign_hook`/`unassign_hook` per literal, so a desynced theory frame is **unrepresentable** (the "make the bug class impossible, not guarded" thesis). It is the **production default**; the legacy `TheoryCallback` is an opt-out (`(set-option :oxiz.use-hooks-driver false)`) cross-check fallback, and the four `arith` stale-bound suppression guards — empirically *dead* on the lock-step path (z3-diff fatal=0 with them off) — are gated to it. Phase 2b kept perf parity by running the expensive euf/arith consistency battery only at full assignment (`final_check_complete`), not every fixpoint (~100× on the harder suites). **A pre-existing engine-wide EUF↔arith combination bug fixed**: a term arith-FIXED by bounds (`f(1)>=5 ∧ f(1)<=5 ⟹ f(1)=5`) is now propagated into EUF so congruence fires (`f(f(1))=f(5)`), eliminating a class of spurious `sat`; the conflict clause is **augmented** with the fixing bounds so the dual never becomes a spurious `unsat` (`fixed_value_with_reasons` scratch-probe + `interned_term_ids` + `pending_arith_eq_reasons`). Plus a `Neg(IntConst n)→IntConst(-n)` literal **sanitizer** (`mk_neg`) + a printer **unsanitizer** (`int_literal_smtlib`, negative `IntConst` → `(- n)`) so SMT-LIB `(- 3)` reaches every `IntConst` fast-path (incl. the EUF constant-node interning). Validated to a high bar: z3 differential on **both** drivers (8000+ random EUF+LIA/arith + a **60-case z3-verified adversarial corpus** `arith_euf_fixed_value_adversarial.rs` — off-path decoys, distinct-value diamonds, multiple merge-classes, deep chains, negatives) — **zero spurious `unsat`**; OxiZ suites green (core 1181 / theories 1383 / solver 756). **Impact on verus-fork's open prelude P0** (`lu-smt --features oxiz`): trigger **E** (int-width `uInv`/`iInv`/`charInv` + `has_type`, distinct-free) now **`sat`** (matches z3 — exactly the EUF↔arith case this fixes); trigger **D** now **`unknown`** (sound); the full prelude **no longer returns a spurious `unsat`** (it times out, as z3 also does — the in-process-OxiZ prelude-scale cost of the new per-term fixed-value probe is a follow-up). adsmt-side: submodule pointer bumped; the redesign is OxiZ-side so the adsmt-workspace test count is unchanged. **1105** green. |
| rc.36 | **`:abduct-theory`'s per-candidate `check-sat` delegates through OxiZ — and the vendored OxiZ is fixed to make it correct.** verus-fork found that `:abduct-theory` (rc.35.1) worked on native arithmetic (`x>0 ∧ y>0 ⊨ x+y>0`) but returned `[]` on a *real* obligation `(> (Add x y) 0)`, because `Add` is an axiomatized function (`∀a b. Add(a,b)=a+b :pattern …`) and the per-candidate `check-sat` ran on the **native** engine — `unknown` there, and it didn't delegate. rc.36 routes the entailment (`F ∧ H ∧ ¬G` UNSAT) and consistency (`SAT(F ∧ H)`) checks through the **same `oxiz_fallback` delegation the top-level `(check-sat)` uses** (`Driver::decide_fh`; native first, OxiZ on an undecided verdict; the buffer is stripped of the adsmt-specific abductive commands OxiZ can't parse). **But the vendored OxiZ returned *wrong* verdicts on that exact axiom** (z3-cross-checked), so the delegation needed three OxiZ-side fixes (submodule `external/oxiz`, branch `0.2.4-feat/cdqi`): (1) **UF-of-int sort persistence** — `Context::execute_script` re-parsed each command with a fresh parser whose declared-function table was empty, so a fed-one-at-a-time `(f 3)` defaulted to `Bool` and went invisible to EUF/LIA; now a `ParserEnv` is threaded across calls (the State-monad shape). (2) **IntConst pinned apart on the EUF equality path** (mirroring the BV arm), so `f(3)=3 ∧ f(3)=4` is the conflict it should be. (3) **pattern-guided MBQI** — `:pattern` triggers are threaded into `Forall`, e-matching runs to a fixpoint first, the model-based pass skips trigger-annotated axioms and enumerates only trigger-free ones (+ a wall-clock non-termination backstop) — so `Add(2,3)=5 → sat`, the countermodel `y>0 ∧ ¬(Add(x,y)>0) → sat`, the contradiction `→ unsat`, all matching z3. The in-process delegation now returns the `(>= x 0)` abduct on the Verus `Add` encoding in ~0.01 s. **CDQI** (conflict-driven quantifier instantiation, Reynolds+ FMCAD'14) added on the same branch — conflict instances over existing ground terms before the constructing enumeration. 4 adsmt-side regressions (`theory_abduction_delegation.rs` + `strip_abductive_commands` / `command_head` units); OxiZ-side `uf_sort_and_quant_soundness.rs` (12, incl. CDQI). 1101 → **1105** green. |
| rc.35.1 | **Re-parseable `term` in the abductive JSON.** verus-fork's design for the abductive consumer (verify-or-explain) flagged that the ranked `(abduce)` JSON rendered `hypotheses` via the engine's curried-HOL `Term` Display (`> x 0`) — not re-parseable, so a back-translation step couldn't recover the term. rc.35.1 routes both the per-candidate `hypotheses` AND a new top-level **`term`** field through `term_to_smtlib` (the rc.35 spine-flattener), so each candidate carries a re-parseable SMT-LIB abduct (`(> x 0)`) byte-identical to what `(get-abduct …)` emits in its `(define-fun … Bool <term>)`. A consumer can now list/rank (A2a) and back-translate (A2c) from the **one** ranked-JSON parser instead of two. Command enum unchanged (additive JSON field; wire-compatible). New regression `abductive_json_carries_reparseable_term_and_hypotheses`. 1089 → **1090** green.  **Follow-up — consistency-enforced abduction.** A derivation-only abduct (`H ⊢ G`) can be *inconsistent with the assertions* `F`, and would then verify the goal **vacuously** (a downstream re-check `F ∧ H ⊨ P` passes when `F ∧ H` is unsat), surfacing a misleading suggestion like `requires x > 0 ∧ x < 0`.  Opt-in `(set-option :abduct-consistency true)` gives the full cvc5 `(get-abduct)` semantics — `F ∧ H ⊨ G` **AND** `SAT(F ∧ H)` — by `check-sat`-ing each candidate against the assertion stack engine-side (cheaper than the consumer's N `SmtProcess` round-trips): `(abduce …)` tags each JSON candidate with a `consistent` boolean, `(get-abduct …)` / `(get-abduct-next)` drop the inconsistent ones.  Default off keeps the cheap derivation-only mode.  2 more regressions; CLI-verified (vacuous `x>0` vs `F: x<0` → `(fail)` / `consistent:false`).  1090 → **1091** green.  **Follow-up — streaming robustness.** verus-fork found (wiring A2a) that `(abduce <term>)` with an unparseable / unknown-operator term **exited** lu-smt (code 13) instead of report-and-continue — fatal in the persistent-solver streaming model (one lu-smt per air context; a mid-stream exit makes the reader miss the `<<DONE>>` sentinel and the whole verus run abort).  The abductive commands are read-only queries (they touch neither the assertion stack nor any verdict), so `Driver::recoverable_command_error` now reports the error on stderr and **continues** (the cvc5/z3 `-in` contract); `--strict-commands` batch mode keeps the hard error, and `(assert)` is untouched (dropping a constraint is soundness-sensitive).  Integration test `adsmt-cli/tests/streaming_robustness.rs` spawns the binary on verus-fork's exact repro.  1091 → **1094** green.  No version bump (additive robustness fix).  **Follow-up — theory-aware abductive search.** verus-fork found that the abduce *search* is purely syntactic (SLD / α-match + Horn), so `x>0 ∧ y>0 ⊬ x+y>0` and `x>0 ⊬ x≥1` return [] — i.e. essentially every theory/arithmetic obligation (all of verus's) gets nothing.  Opt-in `(set-option :abduct-theory true)` swaps in a theory-entailment search: a bounded minimal-subset search over the declared abducibles for an `H` with `F ∧ H ⊨ G` under the theory (`= F ∧ H ∧ ¬G` UNSAT — the dual of the `:abduct-consistency` check) that also enforces `SAT(F ∧ H)`, so on its own it is the full cvc5 `(get-abduct)` contract.  Opt-in because it's complementary, not nested — the SLD search reasons over a separate Horn rule base the theory search (which sees only `F`) can't reproduce, and the theory search pays a `check-sat` per candidate subset.  6 integration regressions (the multi-predicate abduct SLD can't find, integer reasoning, vacuous-drop, trivial-`true` when `F ⊨ G`, default-still-SLD, cvc5 define-fun).  1094 → **1100** green.  No version bump.  **Typing refinement** (illegal-states-unrepresentable — adsmt's own `Theorem`-kernel discipline applied to the abductive surface): the search strategy is derived once into a total `AbductMode` enum (`Sld` / `SldConsistent` / `Theory`), so the dispatch is one exhaustive `match` and "theory subsumes consistency" lives in exactly one place — replacing the scattered `if abduct_theory … else …` + `check = consistency && !theory` bool gates; and a `TheoryAbduct` newtype (private fields, sole constructor `verified` runs both `F ∧ H ⊨ G` and `SAT(F ∧ H)`) makes an *unverified* theory abduct **unrepresentable**, so the vacuous-abduct hazard is a compile-time impossibility on that path, not a discipline a future edit could forget.  Pure refactor (behaviour unchanged); 1100 → **1101** green. |
| rc.35 | **Abductive-reasoning SMT-LIB surface — `(get-abduct)` / `(declare-abducible)`.** adsmt's distinctive fourth verdict (`Abductive { candidates }`) was reachable only when the engine escalated to it internally on a `(check-sat)`; there was no way for a front-end to *ask* for an abduct on a specific goal with a declared vocabulary (`(abduce …)` parsed but fell to `Raw` and was ignored). rc.35 wires three commands through the parser → CLI → `Solver`: **`(declare-abducible <pattern> [<explanation>])`** registers a hypothesis the engine may propose (`Solver::register_abducible`); **`(abduce <goal>)`** (adsmt-native) emits the full ranked candidate set as the single-line `abductive` JSON the Verus/Lean reporters already parse; **`(get-abduct <name> <goal>)`** (the cvc5 SMT-LIB abduction extension) emits the top-ranked abduct as a re-parseable `(define-fun <name> () Bool <body>)`, and **`(get-abduct-next)`** walks the rest (adsmt already ranks the whole set, so it's a cursor). A new spine-flattening `term_to_smtlib` renders the abduct body as valid SMT-LIB (`((> x) 0) → (> x 0)`, which the engine's curried HOL Display does not). This is the cheapest path for a verifier to benefit from adsmt's abductive-deductive reasoning: on a failed/`unknown` obligation, ask `(get-abduct)` with the in-scope variables declared as abducibles, and surface the suggestion (treated as a hypothesis the user must justify — never silently assumed). 9 new tests (parser surface + cursor + SMT-LIB rendering); CLI-verified end-to-end. 1083 → **1089** green. |
| rc.34.6 | **AOT-only path un-taxed — gate the prelude atom-map precompute on a loaded trace.** verus-fork measured rc.34.5: the §3.5.J consult marginal `(3) − (2)` hit ≈ 0 ms (goal met) — but the prelude atom-map build had moved into `with_aot_cdcl`, which runs on **every** `--aot-load`, so the AOT-only path (`just verify-adsmt-fast`, any `VERUS_ADSMT_AOT_LUART` without a trace) regressed `~0.019 s → ~0.40 s` building a map only `replay_events` (trace path) ever reads. Fix: the precompute moves to `set_loaded_jit_trace` — built only when a trace is actually installed (`Solver::build_prelude_atom_map`, the fixed prelude sources only); a bare `--aot-load` builds nothing. The JIT path still pays the one-time `O(prelude)` build once per session (amortized across the session's `(check-sat)`s). If the base is somehow absent at consult, the full `live_atom_map` fallback keeps correctness. No wire/bank/`.lutrace` change. New regression `prelude_atom_map_is_built_only_when_a_trace_is_installed`. CLI-verified (AOT-only back to bank-reconstruction cost; consult still short-circuits). 1082 → **1083** green. |
