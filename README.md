...
| What | Where |
|---|---|
| Project version | `1.0.0-rc.35` (testing channel; cuts to `v1.0.0` stable on explicit sign-off) |
...
| Tests | **1089** passing across the workspace; 0 `cargo doc` / `cargo build` warnings (rc.34.x closed the §3.5 JIT-on-AOT trace-replay perf arc — slim trace → clause-set digest → incremental digest → precomputed prelude atom map, all `O(query delta)`; rc.35 added the **abductive-reasoning SMT-LIB surface** — `(declare-abducible …)`, `(abduce …)`, and the cvc5-compatible `(get-abduct …)` / `(get-abduct-next)` — so a verifier can ask *what hypothesis would discharge this?* and walk the ranked answers) |
...
## Workspace topology (v1.0.0-rc.35)
