...
| Tests | **1094** passing across the workspace; 0 `cargo doc` / `cargo build` warnings (rc.34.x closed the §3.5 JIT-on-AOT trace-replay perf arc — slim trace → clause-set digest → incremental digest → precomputed prelude atom map, all `O(query delta)`; rc.35 added the **abductive-reasoning SMT-LIB surface** — `(declare-abducible …)`, `(abduce …)`, and the cvc5-compatible `(get-abduct …)` / `(get-abduct-next)` — so a verifier can ask *what hypothesis would discharge this?* and walk the ranked answers) |
