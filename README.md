...
| What | Where |
|---|---|
| Project version | `1.0.0-rc.34.1` (testing channel; cuts to `v1.0.0` stable on explicit sign-off) |
...
| Tests | **1070** passing across the workspace; 0 `cargo doc` / `cargo build` warnings (rc.34 built the §3.5 JIT-on-AOT trace-replay sub-cycle; rc.34.1 fixed the record↔replay path so the consult actually short-circuits) |
...
## Workspace topology (v1.0.0-rc.34.1)
